...
- Техническая деталь: scrcpy при старте и смене разрешения сам меняет размер своего окна и может выставить override-разрешение виртуального дисплея — EffectAPK удерживает геометрию сторожевым таймером (`ScrcpyHost`) и фиксирует разрешение дисплея после запуска сессии.
