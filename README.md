...
- **Рендер 1:1:** после любого изменения размеров окна (drag, Shift+drag, максимизация, поворот) разрешение виртуального дисплея синхронизируется с пикселями клиентской области, а плотность (`wm density`) масштабируется пропорционально короткой стороне (эталон 1080 px / 320 dpi → ширина всегда ~540 dp). Обычный resize — «зум» без изменения вёрстки, Shift+resize — реальный реflow, и в обоих случаях картинка рендерится без даунскейла.
- Техническая деталь: scrcpy при старте и смене разрешения сам меняет размер своего окна и может выставить override-разрешение виртуального дисплея — EffectAPK удерживает геометрию сторожевым таймером (`ScrcpyHost`) и синхронизирует дисплей с окном после запуска сессии.
