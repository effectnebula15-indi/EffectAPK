...
**Почему scrcpy-протокол, а не окно scrcpy.exe:** `scrcpy-server` (Java-компонент, запускается на устройстве через `app_process`) умеет `new_display=WxH` — создаёт *виртуальный дисплей* и запускает на нём одно приложение (`start_app`). Это даёт картинку без статус-бара/навигации/домашнего экрана Android и независимый дисплей на каждый APK. EffectAPK **не запускает `scrcpy.exe`** и не встраивает чужое окно — вместо этого реализован собственный клиент протокола (см. ниже): так убирается целый класс проблем с «сбегающим» SDL-окном, а картинка становится обычным WPF-элементом.

**Собственный рендер (v0.3):** EffectAPK сам поднимает `scrcpy-server` и открывает три сокета (video/audio/control):
- **video** — H.264 декодируется через FFmpeg (`avcodec-61.dll` и др. берутся из дистрибутибутива scrcpy, отдельная загрузка не нужна), YUV420→BGRA конвертируется вручную (BT.601, параллельно по строкам) и выводится в `WriteableBitmap` со `Stretch=Fill` — рендер 1:1 к пикселям окна, без даунскейла и чёрных полос;
- **audio** — raw PCM 48кГц/16бит/стерео играет через NAudio (`WaveOut`);
- **control** — мышь (touch), колесо (scroll), клавиатура и текст (Unicode) шлются бинарными сообщениями протокола; правый клик = Android BACK, средний = HOME.

**Почему WPF, а не WinUI 3:** нужны низкоуровневый перехват `WM_SIZING`/`WM_SYSCOMMAND`, per-monitor DPI и трей — в WPF это первоклассные механизмы, в WinUI 3 — известные болевые точки. Видеокадр — обычный `WriteableBitmap`, чужие HWND больше не встраиваются.
...
└── src/EffectApk.App/            WinExe, net8.0-windows, WPF (+WinForms только ради NotifyIcon)
    ...
    ├── Media/
    │   └── VideoDecoder.cs       FFmpeg H.264-декодер + YUV420→BGRA (BT.601, параллельно)
    ├── Sessions/
    │   ├── ScrcpyClient.cs       Клиент протокола scrcpy: push server, forward, 3 сокета,
    │   │                         декод видео, аудио через NAudio, ввод в control-сокет
    │   └── AppSession.cs         Связка APK ↔ scrcpy-клиент ↔ окно
    ├── UI/
    │   ├── AppHostWindow.xaml(.cs)  Окно приложения: WM_SIZING (аспект) / Shift+resize / поворот
    │   ├── ScrcpyView.cs         WPF-рендер (WriteableBitmap) + прокидывание мыши/клавиатуры
...
## Поворот и память окна

- **Поворот (портрет ⇄ альбом):** правый клик по заголовку окна → «Повернуть (портрет/альбом)». Разрешение Android-дисплея и окно меняются местами.
- **Память на приложение:** при закрытии окна его позиция, размер и разрешение дисплея сохраняются в `settings.json` и восстанавливаются при следующем открытии того же пакета. Данные самих Android-приложений (логины, настройки) живут в эмуляторе и переживают закрытие окна и перезапуск рантайма.
- **Рендер 1:1:** после любого изменения размеров окна (drag, Shift+drag, максимизация, поворот) разрешение виртуального дисплея синхронизируется с пикселями клиентской области, а плотность (`wm density`) масштабируется пропорционально короткой стороне (эталон 1080 px / 320 dpi → ширина всегда ~540 dp). Обычный resize — «зум» без изменения вёрстки, Shift+resize — реальный реflow, и в обоих случаях картинка декодируется ровно в размер окна (без даунскейла).
- Техническая деталь: смена конфигурации виртуального дисплея (`wm size`/`wm density`) выкидывает приложение на лаунчер вторичного дисплея — EffectAPK возвращает его на передний план через `am start --display` после каждой смены разрешения.
...
|---|---|---|---|
| [scrcpy](https://github.com/Genymobile/scrcpy) | `scrcpy-server` (протокол) + FFmpeg DLL | Apache-2.0 | Скачивается мастером с GitHub Releases |
| [FFmpeg.AutoGen](https://github.com/Ruslan-B/FFmpeg.AutoGen) | C#-биндинги к FFmpeg | LGPL-3.0 | NuGet |
| [NAudio](https://github.com/naudio/NAudio) | Воспроизведение PCM-аудио | MIT | NuGet |
