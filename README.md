...
- **Кратность 8 обязательна:** размер дисплея всегда округляется до кратного 8 (`ResizePolicy.Align8`). scrcpy-server выравнивает под кодек размер *видеопотока*, но не самого дисплея — при некратном размере (например 684×881 → поток 688×880) картинка кодируется со сдвигом, отчего контент «съезжает», а координаты касаний промахиваются мимо кнопок.
...
- **GPU эмулятора.** По умолчанию `-gpu host` (аппаратный рендеринг Android UI). Важно: `-gpu auto` вместе с `-no-window` выбирает SwiftShader — программный рендеринг на CPU, из-за которого картинка ощутимо лагает. Если аппаратный GPU недоступен (нет драйверов, RDP-сессия), EffectAPK автоматически откатывается на `swiftshader_indirect` и запоминает выбор в `settings.json`; в этом режиме плавность ограничена CPU.
